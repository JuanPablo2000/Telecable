...
require(["baja!", "jquery", "bootstrap", "momentsLocales", "scrollbar", "pcoded", "mapLight", "imageMaps"], function(baja, $, bootstrap, moment, scrollbar) {
    ...
    var r = new baja.BatchResolve(["station:|slot:/Services/AlarmService/subestacion_corteSuprema",
        "station:|slot:/Drivers/NiagaraNetwork/Station_CP_P00_Jace01/points/UPS_CorteSuprema/Variables/porcentajeSalidaPromedio_UPS_CS",
        "station:|slot:/Services/AlarmService/UPS_corteSuprema",
        "station:|slot:/Services/AlarmService/AlarmasBombas",
        "station:|slot:/Drivers/ModbusTcpNetwork/SubEstacion_CorteSuprema/points/PotenciaRealTotal_subestacionCS",
        "station:|slot:/Drivers/NiagaraNetwork/Station_CP_P00_Jace01/points/Promedios/Variables/presionAguaPotable_CorteSuprema",
    ...
    function update(property) {
        switch (property.getName()) {
            case "porcentajeSalidaPromedio_UPS_CS":
                console.log(property.getOut().getValue());
                $("#dash02").html(property.getOut().getValue().toFixed(0) + " %");
                gaugeCircle1.set(property.getOut().getValue());
                break;
            case "PotenciaRealTotal_subestacionCS":
                $("#dash01").html("<i class='indi-electrico'></i>" + Math.round(property.getOut().getValue()) + "<span>kW</span>");
                break;
                break;
            case "presionAguaPotable_CorteSuprema":
                $('#dash03').html("<i class='indi-presion'></i>" + Math.round(property.getOut().getValue()) + "<span>psi</span>");
                break;

            case "subestacion_corteSuprema":
                (property.getInAlarmCount()) > 0 ? $('#alarmSubestacion').addClass('iconAlarm') : $('#alarmGen').removeClass('iconAlarm');
                break;
            case "UPS_corteSuprema":
                (property.getInAlarmCount()) > 0 ? $('#alarmUPS').addClass('iconAlarm') : $('#alarmTransformador').removeClass('iconAlarm');
                break;

            case "AlarmasBombas":
                (property.getOut().getValue()) ? $('#AlarmasBombas').addClass('iconAlarm'): $('#alarmPanelIncendio').removeClass('iconAlarm');
                break;
                case "L001M058":

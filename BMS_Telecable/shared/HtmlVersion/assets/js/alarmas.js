...
require(["baja!", "jquery", "bootstrap", "moment", "pdfmaker", "vfs_font", "dateDropper", "scrollbar", "pcoded", "bootstrapTable", "responsiveBootstrap", "buttonsBootstrap", "buttonsHtml", "datetime-moment"], function(baja, $, bootstrap, moment) {
        ...
        sub.attach('changed', function(prop) {
            switch (prop.getName()) {
                ...
                case "inAlarmCount":
                    ...
                    baja.Ord.make("alarm:|bql:select from openAlarms where alarmClass!='defaultAlarmClass'").get({
                        cursor: {
                            before: function(a) {
                                t.clear().draw();
                            },
                            each: function(a) {
                                ar = [{ n: "alarmData", d: a.get("alarmData") }, { n: "normalTime", d: getNormalTime(a.get("normalTime")) }, { n: "uuid", d: a.get("uuid").toString() }, { n: "alarmClass", d: a.get("alarmClass") }, { n: "timestamp", d: a.get("timestamp") }, { n: "lastUpdate", d: a.get("lastUpdate") }, { n: "ackState", d: a.get("ackState").toString() }];

                                tempArr = (transArr(ar));

                                if (typeof tempArr.alarmData.enterprise !== 'undefined' && tempArr.ackState == 'Acked') {
                                    tempArr.state = 1;
                                } else if ((tempArr.normalTime == null) && tempArr.ackState == 'Acked') {
                                    tempArr.state = 4 //color amarillo
                                } else if (tempArr.normalTime == null) {
                                    tempArr.state = 3 //color rojo
                                } else if (tempArr.normalTime != null && ((tempArr.ackState == 'Unacked') || (tempArr.ackState == 'Ack Pending'))) {
                                    tempArr.state = 2 //color verde
                                } else {
                                    tempArr.state = 1
                                }

                                if (tempArr.ackState == 'Acked') {
                                    tempArr.reconocer = '<span class="text-success"><i class="icon feather icon-check-circle m-r-5"></i>ack</span>'
                                } else {
                                    tempArr.reconocer = '<button class="btn btn-round btn-danger" onclick="arr.setAck(\'' + tempArr.uuid + '\')"><i class="icon feather icon-check m-r-5"></i>ack</button>'
                                }
                                if (tempArr.state != 1) {
                                    t.row.add({
                                        id: tempArr.uuid,
                                        alarmClass: tempArr.alarmClass,
                                        timestamp: moment(Date.parse(tempArr.timestamp)).format('MMM D, YY HH:mm:ss', 'es'),
                                        lastUpdate: moment(Date.parse(tempArr.lastUpdate)).format('MMM D, YY HH:mm:ss', 'es'),
                                        msgText: checkUndefined(tempArr.alarmData),
                                        ackState: '<span style=color:' + colorPriority(tempArr.state) + '><i class="fas fa-bell fa-lg"></i></span>',
                                        ack: tempArr.ackState,
    ...
    function getNormalTime(time) {
        ...
        (time.getDate().getYear() != new Date().getFullYear()) ? obj = null: obj = time;
        return obj;
    }
    ...
    function ackAlarms() {
        ...
        t.rows().every(function(rowIdx, tableLoop, rowLoop) {
            (this.data().ack == 'Unacked' || this.data().ack == "Ack Pending") ? a.ids.push(this.data().id): null
        })
        baja.Ord.make("alarm:").get({
            ok: function(e) {
                this.ackAlarms(a);
    ...
    function transArr(arr) {
        ...
        for (i = 0; i < arr.length; i++) {
            (arr[i].n == "alarmData") ? obj[arr[i].n] = transString(arr[i].d.toString()): obj[arr[i].n] = arr[i].d;
